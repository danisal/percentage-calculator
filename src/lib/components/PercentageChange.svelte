...
<div>
	...
	<div class="flex items-center gap-3 my-3">
		...
		<span class="text-zinc-500 text-sm">{i18n.t.percentChange.toConnector}</span>
	</div>
		...
		<div class="border-t border-stone-300 dark:border-zinc-700 mt-3 pt-3">
			<p class="text-sm text-zinc-500 dark:text-zinc-400">
				{#if result !== null && result !== 'division_by_zero'}
					{i18n.t.percentChange.summaryFrom} {fromInput.trim() || '…'} {i18n.t.percentChange.summaryTo} {toInput.trim() || '…'}: {result > 0 ? '+' : ''}{formatResult(result)}% {i18n.t.percentChange.summaryChange}
				{:else}
					—
				{/if}
			</p>
